...
fmsg defines a protocol with structured binary messages where messages can link to a previous message using a cryptographic hash, forming verifiable threads. Messages are sent via a sender's domain host to one or more recipient hosts. Receiving hosts verify message size, recipient policy and message ancestry before accepting message content. The protocol permits only participants in a message can reply to that message, and allows participants in a message to add additional recipients after it has been sent.
...
This document describes the goals, principles and core architecture of fmsg. A full experimental specification including wire format, protocol steps and implementation is available at [https://github.com/markmnl/fmsg](https://github.com/markmnl/fmsg). The purpose of this document is to describe the architecture and solicit IETF venue and scope feedback before the full specification is split into applicable standards-track documents.
